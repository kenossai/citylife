<x-app-layout>
@section('title', 'Bible School ' . $year)
@section('description', 'Browse events and resources from ' . $year . '.')

<section class="page-header">
    <div class="page-header__bg" style="background-image: url('{{ asset('assets/images/backgrounds/page-header-bg-1-1.jpg') }}');"></div>
    <div class="container">
        <h2 class="text-white">Archive</h2>
        <h2 class="page-header__title">Bible School {{ $year }}</h2>
        <p class="section-header__text">Browse events and resources from {{ $year }}.</p>
                ...
                <div class="product__info-top">
                    <div class="product__showing-text-box">
                        <p class="product__showing-text">
                            Showing {{ $events->count() }} of {{ $events->count() }} Results for {{ $year }}
                        </p>
                    </div>
                    <div class="product__showing-sort">
                        <form method="GET" class="d-inline">
                            <select name="sort" class="form-select" onchange="this.form.submit()" aria-label="Sort by">
                                <option value="date_desc" {{ request('sort') == 'date_desc' ? 'selected' : '' }}>Sort by latest date</option>
                                <option value="date_asc" {{ request('sort') == 'date_asc' ? 'selected' : '' }}>Sort by oldest date</option>
                    ...
                    @empty
                        <div class="col-12">
                            <div class="text-center py-5">
                                ...
                                <p>There are no Bible School events for {{ $year }}.</p>
                            </div>
                        </div>
                    @endforelse
                </div>
            ...
            <div class="col-lg-3">
                <aside class="product__sidebar">
                    <div class="product__search-box product__sidebar__item">
                        <form action="{{ route('bible-school-international.archive', $year) }}" method="GET" class="product__search">
